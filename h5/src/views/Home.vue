  ...
  <div class="home">
    ...
    <div class="hero">
      ...
      <div class="hero__search" @click="$router.push('/rooms')">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><circle cx="11" cy="11" r="7" stroke="currentColor" stroke-width="2"/><line x1="20" y1="20" x2="16.2" y2="16.2" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
        <span>搜索房型、设施...</span>
      </div>
    </div>
    ...
    <div class="quick">
      ...
      >
        <div class="quick__icon" :style="{ background: item.bg }">{{ item.icon }}</div>
      ...
      <div v-else class="room-list">
        ...
        >
          <div class="room-card__img-wrap">
            ...
            <div v-else class="room-card__img room-card__img--ph">🛏️</div>
          </div>
          <div class="room-card__body">
            ...
            <div class="room-card__footer">
              ...
              <span class="room-card__btn">预订 →</span>
            </div>
          </div>
        </div>
      </div>
...
const quickItems = [
  { path: '/rooms', icon: '🛏️', label: '客房预订', bg: 'rgba(0,212,255,.12)' },
  { path: '/orders', icon: '📋', label: '我的订单', bg: 'rgba(0,255,136,.12)' },
  { path: '/member', icon: '💎', label: '会员中心', bg: 'rgba(168,85,247,.12)' },
  { path: '/mall', icon: '🎁', label: '积分商城', bg: 'rgba(255,51,102,.12)' },
];
...
.hero__grid {
  position: absolute;
  inset: 0;
  background: linear-gradient(90deg, rgba(0,212,255,.03) 1px, transparent 1px),
...
.section__more { font-size: 12px; color: var(--neon-cyan); cursor: pointer; }
...
.room-card__btn { font-size: 12px; color: var(--neon-purple); font-weight: 600; transition: color var(--dur-fast); }
